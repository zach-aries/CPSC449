{-
  I believe most of my program logic for the additional challenge is correct,
  but I get a weird non-exahustive pattern I cannot figure out
-}
...
--
data QuadTree a = Leaf (Int, Int, Int)
              | Node (Int,Int) (QuadTree a) (QuadTree a) (QuadTree a) (QuadTree a)
              deriving (Eq, Show)

data QuadTreeImage  = QuadTreeImage Int Int (QuadTree (Int))
                    deriving (Show)
...
--
averageLeafRGB :: QuadTree a -> QuadTree a
...
--   QuadTree: The resulting QuadTree after the function has been applied
--
mirrorNodes :: QuadTree a -> QuadTree a
mirrorNodes (Node e a b c d) = Node e b a d c -- flip the nodes along the y axis  so  NE NW -> NW NE
...
createTree x
  ...
  | isHomogenous x = QuadTreeImage h w (createLeaf (head (head x)))   -- if the image is homogenous then just make entire thing one colour
  | otherwise = QuadTreeImage h w (createNode x)                      -- otherwise create a new node
  where
    h = length x
    w = length (head x)
...
--   QuadTree
--
createNode :: [[(Int,Int,Int)]] -> QuadTree a
createNode x
  | isHomogenous x = createLeaf (head (head x))   -- base case: if array passed is homogenous then we make it a leaf
  | otherwise = Node (h,w) (createNode nw) (createNode ne) (createNode sw) (createNode se) -- otherwise create a new node and recursively call
  where                   -- split Node into 4 quadrants NW,NE,SW,SE this is represented in each Node of the quad tree
    split_h = (h `div` 2) -- for squares we can just split in half
    split_w = (w `div` 2) -- for squares we can just split in half
    h = length x
    w = length (head x)
    nw = sub2DArray x 0 (split_h - 1) 0 (split_w - 1) 0
    ne = sub2DArray x 0 (split_h - 1) split_w (w-1) 0
    sw = sub2DArray x split_h (h-1) 0 (split_w - 1) 0
    se = sub2DArray x split_h (h-1) split_w (w-1) 0
...
--   QuadTree Leaf
--
createLeaf :: (Int,Int,Int) -> QuadTree a
...
toHTML (QuadTreeImage h w t) = prefix ++ (parseTree 0 0 h w t) ++ suffix
  where
...
--  String: SVG value
--
parseTree :: Int -> Int -> Int -> Int -> QuadTree a -> String
parseTree x y h w (Leaf (r,g,b)) = rectangle x y h w (r,g,b)
parseTree x y h w (Node a nw ne sw se) =  (parseTree x y split_h split_w nw) ++
                                          (parseTree (x + split_w) y split_h split_w ne) ++
                                          (parseTree x (y + split_h) split_h split_w sw) ++
                                          (parseTree (x + split_w) (y + split_h) split_h split_w se)
  where
    split_h = (h `div` 2)
    split_w = (w `div` 2)
...
--   QuadTree: The resulting QuadTree after the function has been applied
--
treeMap :: (QuadTree a -> QuadTree a) -> QuadTree a -> QuadTree a
treeMap f (Leaf rgb) = f(Leaf rgb)
treeMap f (Node e a b c d) = f( Node e (treeMap f a) (treeMap f b) (treeMap f c) (treeMap f d))
...
grayscale (QuadTreeImage h w q) = QuadTreeImage h w (treeMap averageLeafRGB q)
...
mirror (QuadTreeImage h w q) = QuadTreeImage h w (treeMap mirrorNodes q)
...
main = do
  ...
  writeFile "quadtree_additional.html" (prefix ++
                            (toHTML qtree_image) ++
                            suffix ++
                            "<br><br><br>" ++
                            (toHTML gs) ++ "<br><br><br>" ++
